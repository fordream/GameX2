                        ...
                        const _GLFWfbconfig* fbconfig)
{
	...
	//window->win32.dwExStyle = WS_EX_APPWINDOW;//WS_EX_APPWINDOW;
    ...
    else
    {
		if (wndconfig->decorated)
		{
			...
			window->win32.dwStyle |= WS_POPUP;
		//WS_EX_TOOLWINDOW 无一口X

		//window->win32.dwExStyle = WS_EX_TOOLWINDOW | WS_POPUP;
		//window->win32.dwExStyle = WS_EX_APPWINDOW;
		//window->win32.dwStyle = WS_POPUP | WS_SYSMENU;

		window->win32.dwStyle |= WS_EX_TRANSPARENT | WS_EX_TOPMOST;
	...
	if (!wndconfig->decorated)//无边框的窗口设置，改为FBO模式
	{
		window->win32.dwExStyle = WS_EX_LAYERED;
		window->win32.dwStyle = WS_VISIBLE | WS_POPUP;
	}
	...
	//	修改，隐藏状态栏图标
	if (!wndconfig->decorated){
		SetWindowLong(window->win32.handle, GWL_EXSTYLE, WS_EX_TOOLWINDOW);
		//SetLayeredWindowAttributes(window->win32.handle, RGB(0,0,0), 20, LWA_ALPHA);
	}

	//SetWindowLong(window->win32.handle, GWL_EXSTYLE, GetWindowLong(window->win32.handle,GWL_EXSTYLE)|WS_EX_LAYERED);
	//SetLayeredWindowAttributes(window->win32.handle,RGB(0,0,0), 100, LWA_COLORKEY);//alpha = 240，范围0~255；也可以使用关键色，详见MSDN
	//UpdateWindow(window->win32.handle);

	//HRGN hrgn = CreateRoundRectRgn(0, 0, 1000, 1000, 1000, 1000);
	//SetWindowRgn(window->win32.handle, hrgn, 1);
